function [ Kip ] = build_kip( entry, exit, nhat, P_all, GP,options )
...
while(~isempty(R_set))
    ...
    % L = S(:,end) - S(:,1);
    N1 = repmat(nhat(1,set).^2,n_test,1,i_res);
    N12 = repmat(2*nhat(1,set).*nhat(2,set),n_test,1,i_res);
    N2 = repmat(nhat(2,set).^2,n_test,1,i_res);
    
    A = bsxfun(@minus,P_all(:,1),Rx(:)');
    A = reshape(A,n_test,n_s,[]);
    B = bsxfun(@minus,P_all(:,2),Ry(:)');
    B = reshape(B,n_test,n_s,[]);
    ...
    Kpi(1:3:end,set) = Kpi1;
    Kpi(2:3:end,set) = Kpi2;
    Kpi(3:3:end,set) = Kpi3;
    if strcmp(options,'on')
        toc
    end
end
