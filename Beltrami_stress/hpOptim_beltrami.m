function [LogL,grad] = hpOptim_beltrami(theta,num_basis,A,B,C,D,E,F,nu,entry,exit,y,sig_m,nsegs)
...
%     SF = F.sig_f^2*(2*pi)^(3/2)*F.lx*F.ly*F.lz*exp(-0.5*(lambdas(1,5*nm+1:6*nm).^2*F.lx^2+lambdas(2,5*nm+1:6*nm).^2*F.ly^2+lambdas(3,5*nm+1:6*nm).^2*F.lz^2));
    ...
    if exist('nsegs','var')
        [Phi, SLambda,lambdas] = beltrami_approx(num_basis,A,B,C,D,E,F,nu,entry,exit,[],[],[],nsegs);
    else
        [Phi, SLambda,lambdas] = beltrami_approx(num_basis,A,B,C,D,E,F,nu,entry,exit);
    end
end
